...
public class FishSpawner_RTL : MonoBehaviour
{
    [System.Serializable]
    public class Fish
    {
    ...
    public float startX = 9f;    // spawn on the right
    public float endX = -9f;     // travel to the left
    ...
    IEnumerator SpawnLoop()
    {
        while (true)
        {
            ...
            // place at random Y position on right side
    ...
    IEnumerator MoveFish(Transform fish, float speed)
    {
        while (fish != null)
        {
            fish.position += Vector3.left * speed * Time.deltaTime;
